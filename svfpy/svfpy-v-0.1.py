...
class SVF:
    ...
    def pad_max_by_resolution_fitted(self, col, row, resolution):
        col_s, col_f, row_s, row_f = self._col_row(row, col, resolution)
        mds = self.get_downscale(resolution)[0, row_s:row_f, col_s:col_f]
        mds = np.where(mds > 0, mds, 0.)
        delta = mds.max() - mds.min()

        pad_tangent = (self.tangents[::-1][np.max(np.where(self.downscales() * self.resolution == resolution))] * delta) / resolution
        pad_max = np.ceil(((self.kernel_size_side / 2) + pad_tangent) * np.sqrt(2) - (self.kernel_size_side / 2)) 
        return int(pad_max)
        # return mds, delta, pad_tangent, pad_max
    ...
    def _k_slice(self, row, col, resolution):
        ...
        pad_pixels = self.pad_max_by_resolution_fitted(col, row, resolution)

        if pad_pixels > self.pad_max_by_resolution()[resolution]:
            pad_pixels = self.pad_max_by_resolution()[resolution]
        ...
        return (pad_left, pad_right), (pad_top, pad_bottom), (row_s_padded), (row_f_padded), (col_s_padded), (col_f_padded), pad_pixels


    def working_kernel(self, row, col, resolution):
        # REtorna o kernel de trabalho com PAD
        k_slice = self._k_slice(row, col, resolution)
        (pad_left, pad_right), (pad_top, pad_bottom), (row_s_padded), (row_f_padded), (col_s_padded), (col_f_padded), pad_pixels = k_slice
        mds = self.get_downscale(resolution)
        ...
        return k_slice, w_kernel_padded, pad_pixels

    def kernels(self, resolution):
        mds = self.get_downscale(resolution)

        rows = (mds.shape[1] - 1) // self.kernel_size_side
        cols =  (mds.shape[2] - 1) // self.kernel_size_side 
